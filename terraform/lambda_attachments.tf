# S3 Bucket for Ticket Attachments (Milestone 5)
resource "aws_s3_bucket" "attachments_bucket" {
  bucket_prefix = "ticketdesk-attachments-"
  force_destroy = true

  tags = {
    Name        = "${var.resource_prefix}-attachments-bucket"
...
# Zip package for Lambda function (index.js & package.json)
data "archive_file" "lambda_zip" {
  type        = "zip"
  source_dir  = "${path.module}/../lambda"
  output_path = "${path.module}/../lambda/thumbnail.zip"
  excludes    = ["thumbnail.zip"]
}
...
resource "aws_lambda_function" "thumbnail" {
  ...
  tags = {
    Name        = "${var.resource_prefix}-thumbnail-lambda"
    Environment = var.environment
  }
}

# Permission allowing S3 to invoke Lambda
